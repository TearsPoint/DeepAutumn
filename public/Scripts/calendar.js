...
(function () {
    ...
    CalendarWidget.initialize = function (o) {
        //日历层唯一
        if ($(".eui-calendar").size() > 0) {
            content = $(".eui-calendar").first()[0];
            eui.addHandler(o.input, 'focus', onshow);
            return;
        }
        ...
        function onputdate(e) {
            ...
            if (data === 'day') {
                ...
                currentInput.value = date.year + '-' + date.month + '-' + date.date;
                _hide(content);
            }

            if (!eui.isContent(target,content) && target !== currentInput) {
                _hide(content);
            }
        }

        function onshow(e) {
            _setPosition(content, currentInput);
            _show(content);
        }
...
this.getElemLeft = function (element) {
...
this.getElemTop = function (element) {
    if (element == null) return;
...
function _setPosition(elem, input) {
    ...
    var left = getElemLeft(input),
    top = getElemTop(input),
    height = input.offsetHeight;

    $(elem).css('left', left + 'px');
    $(elem).css('top', top + height + 5 + 'px');
}
 
(function () {
    eui.addHandler(window, 'load', function () {
        $(".datetime").focus(function () { currentInput = this; });
        $(".datetime").each(
    ...
    eui.addHandler(window, 'resize', function () {
        if ($(".eui-calendar").size() > 0) {
            _setPosition($(".eui-calendar")[0], currentInput);
        }
    });
})();